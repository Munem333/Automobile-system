...
class BleAdvertiseService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        startForeground(NOTIF_ID, buildNotification("BLE Ready"))
        broadcastStatus("advertising")

        val adapter = getSystemService(BluetoothManager::class.java)?.adapter
        if (adapter == null || !adapter.isEnabled) {
            updateNotification("Turn on Bluetooth to receive orders")
    ...
    private fun setAdapterName(adapter: BluetoothAdapter) {
    ...
    private fun startAdvertising() {
        ...
        if (advertiser == null) {
            updateNotification("BLE advertising not supported on this device")
        ...
        advertiseCallback = object : AdvertiseCallback() {
            ...
            override fun onStartFailure(errorCode: Int) {
                ...
                updateNotification("BLE advertise failed ($reason)")
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            if (ContextCompat.checkSelfPermission(this, android.Manifest.permission.BLUETOOTH_ADVERTISE)
                ...
                updateNotification("Grant Bluetooth Advertise permission")
                return
            }
        }
